...
def validate_order(
    symbol: str, price: float, quantity: float, current_price: float
) -> tuple[bool, str]:
    """
    ...
    rounded_quantity = math.floor(quantity * (10**precision_decimals)) / (
    ...
    logger.info(
        f"[{symbol}] Order Validated: {rounded_quantity} @ {price} (Notional: {notional:.2f} USDT)"
    )
    return True, "Valid"
