...
class Config:
    ...
    BINANCE_SECRET_KEY = os.getenv("BINANCE_API_SECRET") or os.getenv(
        "BINANCE_SECRET_KEY"
    )
    ...
    RISK_MAX_POSITION_PER_SYMBOL = float(
        os.getenv("RISK_MAX_POSITION_PER_SYMBOL", "0.1")
    )
    ...
    GRID_MAX_LEVELS = int(
        os.getenv("GRID_MAX_LEVELS", "20")
    )  # Prevent excessive orders in low vol
    GRID_ATR_MULTIPLIER = float(
        os.getenv("GRID_ATR_MULTIPLIER", "0.5")
    )  # Base multiplier 'k' para grid_spacing = k * ATR

    KILL_SWITCH_ENABLED = os.getenv("KILL_SWITCH_ENABLED", "True").lower() == "true"
    COOLDOWN_MINUTES = float(os.getenv("COOLDOWN_MINUTES", 5.0))
    MIN_NOTIONAL = float(
        os.getenv("MIN_NOTIONAL", 100.0)
    )  # Testnet often requires 100+
