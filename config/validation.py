...
def validate_config():
    ...
    try:
        ...
        if leverage < 1:
            ...
            errors.append(f"MAX_LEVERAGE={leverage} exceeds Binance maximum (125).")
        elif leverage > 50:
            warnings.append(f"MAX_LEVERAGE={leverage} is very high. Consider <= 20.")
    except ValueError:
    ...
    try:
        ...
        if daily_loss <= 0 or daily_loss > 1:
            errors.append(f"MAX_DAILY_LOSS={daily_loss} is out of range (0, 1].")
    except ValueError:
    ...
    tg_token = _env("TELEGRAM_TOKEN") or _env("TELEGRAM_BOT_TOKEN", "")
    tg_chat = _env("TELEGRAM_CHAT_ID")

    if tg_enabled:
        if not tg_token or tg_token == "your_bot_token_from_botfather":
            errors.append(
                "TELEGRAM_ENABLED=true but TELEGRAM_TOKEN/TELEGRAM_BOT_TOKEN is missing.\n"
                "  → Provide a valid token or set TELEGRAM_ENABLED=false."
            )
        else:
            import re

            if not re.match(r"^\d+:[a-zA-Z0-9_-]+$", tg_token):
                errors.append(
                    "TELEGRAM_TOKEN format is invalid.\n"
                    "  → Must match Telegram bot token pattern (e.g. '1234:ABC...')."
                )

        if not tg_chat or tg_chat == "your_chat_id":
            errors.append(
                "TELEGRAM_ENABLED=true but TELEGRAM_CHAT_ID is missing.\n"
                "  → Provide a valid chat id or set TELEGRAM_ENABLED=false."
            )
    ...
    print(
        f"       Leverage: {_env('MAX_LEVERAGE', '3')}x, "
