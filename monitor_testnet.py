...
try:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT * FROM trades WHERE closed_at LIKE ? ORDER BY closed_at DESC",
            (f"{today_str}%",),
        )
        trades = cur.fetchall()
    ...
    if not trades:
        ...
        for t in trades[:10]:
            print(
                f"  {str(t['trade_id']):<15}  {t['symbol']:<10}  {t['side']:<5}  {float(t['realized_pnl']):>+10.4f}"
            )
...
try:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*), SUM(realized_pnl), MAX(realized_pnl), MIN(realized_pnl) FROM trades"
        )
