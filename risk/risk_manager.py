...
class RiskManager:
    ...
    def _check_daily_reset(self, current_equity=0.0):
        ...
        if today != self.last_reset_date:
            logger.info(
                f"[Risk] Daily reset: PnL {self.daily_pnl:.2f} -> 0.00 (new day: {today})"
            )
            self.daily_pnl = 0.0
        ...
        if os.path.exists(self.lock_file):
            try:
                ...
                if lock_date < date.today():
                    logger.info(
                        f"[Risk] Obsolete Kill Switch lock found from {lock_date}. Auto-resetting for new day."
                    )
                    os.remove(self.lock_file)
                    self.is_kill_switch_active = False
                else:
                    logger.critical(
                        f"[Risk] HARD LOCK ACTIVE. Kill Switch was triggered today ({lock_date}). Trading forcibly blocked."
                    )
                    self.is_kill_switch_active = True
            except Exception as e:
                logger.error(
                    f"[Risk] Corrupted lock file: {e}. Defaulting to safe (Locked)."
                )
                self.is_kill_switch_active = True
        ...
        try:
            with open(self.state_file, "r") as f:
                ...
                if not self.is_kill_switch_active:
                    self.is_kill_switch_active = bool(
                        state.get("is_kill_switch_active", False)
                    )
    ...
    def sync_reference_equity(self, equity: float, unrealized_pnl: float):
        ...
        if equity <= 0:
            if not self.is_safe_mode:
                logger.critical(
                    f"[Risk] INVALID EQUITY DETECTED: {equity}. ACTIVATING SAFE MODE."
                )
    ...
    def calculate_position_size(
        self, symbol: str, entry_price: float, stop_loss: float, exchange_client=None
    ) -> float:
        """
        ...
        if (
            equity <= 0
            or self.is_safe_mode
            or self.is_high_caution
            or self.is_kill_switch_active
        ):
            reason = (
                "Equity <= 0"
                if equity <= 0
                    ...
                    if self.is_safe_mode
                        ...
                        "High Caution" if self.is_high_caution else "Kill Switch Active"
                    )
                )
            )
        ...
        if not isinstance(entry_price, (int, float)) or (
            stop_loss is not None and not isinstance(stop_loss, (int, float))
        ):
            etype_entry = type(entry_price).__name__
            etype_sl = type(stop_loss).__name__
            logger.error(
                f"[Risk] TypeError Prevention: entry_price={etype_entry}, stop_loss={etype_sl}. Blocking trade."
            )
            return 0.0
        ...
        if exchange_client:
            is_valid, reason = exchange_client.validate_order_filters(
                symbol, amount, entry_price
            )
            if not is_valid:
                logger.warning(
                    f"[Risk] {symbol} Order REJECTED by exchange filters: {reason}"
                )
                return 0.0
            else:
    ...
    def enforce_inventory_limits(
        self, symbol: str, signal, current_positions: dict
    ) -> float:
        """
        ...
        allowed_new_notional = min(
            new_order_notional, remaining_symbol_notional, remaining_total_notional
        )

        if allowed_new_notional <= 0:
            logger.warning(
                f"[Risk] {symbol} {signal.action.value} blocked. Exposure limit reached. "
    ...
    def check_daily_drawdown(self, current_pnl, equity):
        ...
        if self.daily_pnl <= warning_threshold and self.daily_pnl > limit:
            logger.warning(
                f"[Risk] ⚠️ Drawdown at 50% of limit: PnL={current_pnl:.2f}, Limit={limit:.2f} (Ref={ref_equity:.2f})"
            )

        if self.daily_pnl <= limit:
            if not self.is_kill_switch_active:
                logger.critical(
                    f"Daily Kill Switch Triggered! PnL={current_pnl:.2f} <= Limit={limit:.2f}"
                )
                self.is_kill_switch_active = True
