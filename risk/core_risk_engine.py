...
class CoreRiskEngine:
    ...
    def validate_order(self, order: Signal) -> bool:
        ...
        if equity <= 0:
            logger.error(
                f"[RiskEngine] Blocked {order.action.value} on {symbol}: Zero or negative equity.",
                extra={
        ...
        active_positions = [
            ...
            if isinstance(pos, dict) and pos.get("is_active", False)
        ]
        max_open = getattr(self.config, "RISK_MAX_OPEN_POSITIONS", 5)
        # If it's a new entry and we have reached the max open limit
        if (
            order.action in (SignalAction.ENTER_LONG, SignalAction.ENTER_SHORT)
            and symbol not in active_positions
        ):
            if len(active_positions) >= max_open:
                logger.error(
                    f"[RiskEngine] Blocked {order.action.value} on {symbol}: Max open positions ({max_open}) limit breached.",
                    extra={
                        "event": "RiskLimitBreached",
                        "symbol": symbol,
        ...
        if order.action not in (SignalAction.EXIT_LONG, SignalAction.EXIT_SHORT):
            if current_notional + new_notional > max_notional_per_symbol:
                logger.error(
                    f"[RiskEngine] Blocked {order.action.value} on {symbol}: Max exposure per symbol breached. "
                    f"Limit: {max_pos_limit*100}%. Current Notional: {current_notional:.2f}, Attempted Added Notional: {new_notional:.2f}",
                    extra={
                        "event": "RiskLimitBreached",
                        "symbol": symbol,
                        "reason": "MaxPositionExposure",
                    },
                )
                return False
        ...
        if order.action not in (SignalAction.EXIT_LONG, SignalAction.EXIT_SHORT):
            if overall_notional + new_notional > equity * max_total_limit:
                logger.error(
                    f"[RiskEngine] Blocked {order.action.value} on {symbol}: Total max exposure breached. "
                    f"Limit: {max_total_limit*100}% of Equity.",
                    extra={
                        "event": "RiskLimitBreached",
                        "symbol": symbol,
                        "reason": "MaxTotalExposure",
                    },
                )
                return False
    ...
    def should_shutdown(self) -> bool:
        ...
        if drawdown > max_drawdown:
            logger.critical(
                f"[RiskEngine] HARD KILL SWITCH TRIGGERED: Max drawdown breached! {drawdown*100:.2f}% > {max_drawdown*100:.2f}%",
                extra={
        ...
        daily_loss = (
            (start_balance - equity) / start_balance if start_balance > 0 else 0.0
        )
        max_daily_loss = getattr(self.config, "RISK_MAX_DAILY_LOSS", 0.05)

        if daily_loss > max_daily_loss:
            logger.critical(
                f"[RiskEngine] HARD KILL SWITCH TRIGGERED: Max daily loss breached! {daily_loss*100:.2f}% > {max_daily_loss*100:.2f}%",
                extra={
                    "event": "KillSwitchTriggered",
                    "symbol": "SYSTEM",
                    "reason": "MaxDailyLoss",
