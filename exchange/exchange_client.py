...
from common.resilience import retry_async, retry_sync, ServiceHealth
...
class ExchangeClient:
    def __init__(self):
        ...
        self.health = ServiceHealth("Binance", max_failures=10, cooldown_seconds=120)
    ...
    async def fetch_ohlcv(self, symbol, timeframe, limit=500):
        try:
            ...
            if self.public_exchange:
                data = await self.public_exchange.fetch_ohlcv(
    ...
    def _fetch_ohlcv_fallback(self, symbol, timeframe, limit):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(
                    f"Fallback OHLCV failed for {symbol}: {r.status_code} {r.text}"
                )
        except Exception as e:
    ...
    async def _apply_backoff(self):
        ...
        if self.backoff_multiplier > 1.0:
            ...
            delay = (
                min(30, (self.backoff_multiplier - 1) * 2) + jitter
            )  # Caps at 30s + jitter
    ...
    def _manual_request(self, method, endpoint, params=None):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(
                    f"Manual {method} {endpoint} failed: {r.status_code} {r.text}"
                )
                return None
        except Exception as e:
    ...
    async def init(self):
        ...
        async def _load_via_ccxt():
            if self.public_exchange:
                await asyncio.wait_for(self.public_exchange.load_markets(), timeout=15)
            if self.exchange:
                await asyncio.wait_for(self.exchange.load_markets(), timeout=15)
        ...
        result = await retry_async(
            ...
            max_retries=2,
            base_delay=3.0,
            context="ExchangeClient.init (CCXT)",
        )
        if result is None:
            logger.warning(
                "CCXT market loading failed after retries. Attempting manual load."
            )
            await retry_async(
                self._manual_load_markets,
                max_retries=3,
                base_delay=5.0,
                context="ExchangeClient.init (manual)",
            )
    ...
    async def fetch_open_orders(self, symbol=None):
        if not self.health.is_available:
            logger.debug(
                "[Exchange] Skipping fetch_open_orders — service temporarily disabled."
            )
            return []
        if Config.USE_TESTNET:
        ...
        result = await retry_async(
            ...
            base_delay=2.0,
            default=[],
            context=f"fetch_open_orders({symbol})",
        )
        if result is None:
    ...
    async def fetch_my_trades(self, symbol, limit=50):
        if not self.health.is_available:
            logger.debug(
                "[Exchange] Skipping fetch_my_trades — service temporarily disabled."
            )
            return []
        if Config.USE_TESTNET:
        ...
        result = await retry_async(
            ...
            base_delay=2.0,
            default=[],
            context=f"fetch_my_trades({symbol})",
        )
        if result is None:
            self.health.record_failure()
            return []
    ...
    async def fetch_positions(self):
        ...
        if not self.health.is_available:
            logger.debug(
                "[Exchange] Skipping fetch_positions — service temporarily disabled."
            )
            return []
        if Config.USE_TESTNET:
        ...
        async def _fetch():
            result = await self.exchange.fetch_positions()
            self.health.record_success()
            return result
        ...
        result = await retry_async(
            ...
            max_retries=2,
            base_delay=2.0,
            context="fetch_positions",
        )
        if result is None:
            logger.warning("CCXT fetch_positions failed after retries. Trying manual.")
            self.health.record_failure()
            return await self._manual_fetch_positions()
        return result
    ...
    async def fetch_order_book(self, symbol, limit=100):
        ...
        try:
            ...
            logger.warning(
                f"[Exchange] CCXT fetch_order_book failed for {symbol}: {e}. Retrying manually..."
            )
            ob = await self._manual_fetch_order_book(symbol, limit)
    ...
    async def _manual_fetch_order_book(self, symbol, limit=100):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(
                    f"[Exchange] Manual order book fetch failed: {r.status_code} {r.text}"
                )
        except Exception as e:
    ...
    async def fetch_balance(self):
        ...
        if not self.health.is_available:
            logger.debug(
                "[Exchange] Skipping fetch_balance — service temporarily disabled."
            )
        ...
        if result is None:
            logger.warning("CCXT fetch_balance failed after retries. Trying manual.")
            self.health.record_failure()
            return await self._manual_fetch_balance()
        return result
    ...
    async def _manual_fetch_balance(self):
        ...
        for asset in data:
            if asset.get("asset") == "USDT":
                ...
                free = float(
                    asset.get("withdrawAvailable") or asset.get("availableBalance") or 0
                )
                # Double check total is valid
                if total < 0:
                    logger.error(
                        f"[Exchange] CRITICAL: USDT Balance is negative ({total})!"
                    )
                return {"total": {"USDT": total}, "free": {"USDT": free}, "info": asset}
        return {"total": {"USDT": 0.0}, "free": {"USDT": 0.0}, "info": {}}
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params=None):
        ...
        try:
            ...
            if self.sim_mode:
                logger.info(
                    f"[SIM] Created {side} order for {symbol} @ {price or 'MARKET'}"
                )
                return {
            ...
            if Config.USE_TESTNET:
                ...
                return await self._manual_create_order(
                    clean_symbol, type, side, amount_prec, price_prec, params
                )
            ...
            return await self.exchange.create_order(
                ...
                side,
                float(amount_prec),
                float(price_prec) if price_prec else None,
                params,
            )
        except Exception as e:
            ...
            return await self._manual_create_order(
                symbol.replace("/", ""), type, side, amount, price, params
            )
    ...
    async def _manual_set_leverage(self, symbol, leverage):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.warning(
                    f"Set leverage failed for {symbol}: {r.status_code} {r.text}"
                )
        except Exception as e:
