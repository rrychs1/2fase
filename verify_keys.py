...
def verify_keys():
    ...
    use_testnet = (
        trading_env in ["TESTNET", "DEMO"]
        or os.getenv("USE_TESTNET", "True").lower() == "true"
    )
    ...
    if use_testnet:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                # Buscar saldo USDT
                usdt_balance = next(
                    (item for item in data if item["asset"] == "USDT"), None
                )
                if usdt_balance:
                    print(f"   Saldo Wallet: {usdt_balance['balance']}")
                    print(
                        f"   Saldo Cross Wallet: {usdt_balance.get('crossWalletBalance', 'N/A')}"
                    )
                    print(
                        f"   PnL No Realizado: {usdt_balance.get('crossUnPnl', 'N/A')}"
                    )
                else:
                ...
                try:
                    ...
                    r_pos = requests.get(
                        f"{pos_url}?{query_string}&signature={signature}",
                        headers=headers,
                        timeout=10,
                    )
                    if r_pos.status_code == 200:
                        ...
                        if active_positions:
                            for p in active_positions:
                                print(
                                    f"   [POS] {p['symbol']} | Cantidad: {p['positionAmt']} | Entry: {p['entryPrice']} | PnL: {p['unRealizedProfit']}"
                                )
                        else:
                            print("   [OK] No hay posiciones abiertas.")
                    else:
                        print(
                            f"   [ERROR] Error al obtener posiciones: {r_pos.status_code}"
                        )
    ...
    try:
        ...
        if use_testnet:
            print(
                "   Nota: Si la verificación [1] funcionó, ignora este error de CCXT."
            )
