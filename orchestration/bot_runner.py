...
class BotRunner:
    def __init__(self, config=None, exchange=None, risk_manager=None, data_engine=None):
        ...
        self.current_orders = []
        self.current_history = self.db.get_recent_trades(
            limit=100
        )  # Load last 100 from DB on start
    ...
    async def run(self):
        mode_str = (
            "SIM"
            if self.config.TRADING_ENV == "SIM"
            else (
                "Analysis Only (Paper)"
                if self.config.ANALYSIS_ONLY
                else ("Testnet" if self.config.USE_TESTNET else "LIVE")
            )
        )
        logger.info(f"Starting Async Bot Runner [Mode: {mode_str}]...")
        logger.info(
            f"Risk Hierarchy: Kill Switch -> Safe Mode -> Daily Loss -> Cooldown"
        )

        # Start health monitoring background task (logs every 5 min, alerts on anomalies)
        await self.health_monitor.start_periodic_report(
            interval_s=300, telegram=self.telegram
        )

        # Services already verified by startup pipeline — initialize exchange
        try:
            ...
            self.health_monitor.record_api_success("exchange")
            ...
            self.health_monitor.record_api_failure("exchange")
        ...
        try:
            ...
            if self.config.EXECUTION_MODE == "LIVE":
                logger.info(
                    "[Startup Sync] Reconciling memory with Exchange open orders..."
                )
                for s_symbol in self.config.SYMBOLS:
                    try:
                        ...
                        logger.warning(
                            f"[Startup Sync] Failed to sync orders for {s_symbol}: {e}"
                        )

            # 1. Seed Initial Data via REST (Polling) for all symbols
            logger.info("Seeding initial data before WS/Polling loop...")
            for symbol in self.config.SYMBOLS:
                await self.data_engine.fetch_ohlcv(
                    symbol,
                    self.config.TF_GRID,
                    limit=self.config.CANDLES_ANALYSIS_LIMIT,
                )
                await self.data_engine.fetch_ohlcv(
                    symbol,
                    self.config.TF_TREND,
                    limit=self.config.CANDLES_ANALYSIS_LIMIT,
                )
            ...
            if getattr(self.config, "USE_WEBSOCKETS", True):
                ...
                for symbol in self.config.SYMBOLS:
                    self.ws_manager.add_subscription(
                        symbol, [self.config.TF_GRID, self.config.TF_TREND]
                    )
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"[WS] Processed closed candle: {event.symbol} {event.timeframe}"
                        )
                        ...
                        elapsed_ms = (time.time() - start_iter) * 1000
                        self.health_monitor.record_loop(elapsed_ms)
                ...
                while True:
                    ...
                    self.health_monitor.record_loop(elapsed * 1000)
                    self._flush_alerts_and_log(start_iter)
    ...
    async def _heartbeat_loop(self):
        ...
        while True:
            ...
            ws_state = (
                "Connected"
                if getattr(self.ws_manager, "ws", None)
                and not self.ws_manager.ws.closed
                else "Disconnected"
            )
            ...
            bot_system_health.set(
                0
                if self.risk_manager.is_kill_switch_active
                or self.risk_manager.is_safe_mode
                else 1
            )
    ...
    def _handle_loop_error(self, e, mode_str):
        ...
        if isinstance(e, TypeError):
            ...
            logger.debug(
                f"State Types: iteration={type(self.iteration_count)}, exchange={type(self.exchange)}, telegram={type(self.telegram)}"
            )
        logger.error(f"Error in iteration loop: {e}", exc_info=True)
        self.circuit_breaker.report_error()
        ctx = f"Modo: `{mode_str}`\nIteración: `{self.iteration_count}`\n"
        asyncio.create_task(
            self.telegram.critical(
    ...
    def _flush_alerts_and_log(self, start_iter):
        ...
        elapsed = time.time() - start_iter
        logger.info(
            f"Cycle processed | symbols={len(self.config.SYMBOLS)} duration={elapsed*1000:.0f}ms"
        )
    ...
    async def iterate(self, target_symbol=None):
        ...
        self.update_status(
            "Running" if not self.risk_manager.is_high_caution else "HIGH CAUTION"
        )
        ...
        try:
            ...
            if drift_alert:
                alert_msg = (
                    f"⚠️ **ALERTA DE SEGURIDAD: Deriva de Equity**\n"
            ...
            if self.risk_manager.needs_reconciliation(self.iteration_count):
                logger.info(
                    f"[Bot] Starting Periodic Reconciliation (Iteration {self.iteration_count})"
                )
                for s_symbol in self.config.SYMBOLS:
                    open_orders = await self.exchange.fetch_open_orders(s_symbol)
                    self.neutral_grid.reconcile_with_exchange(s_symbol, open_orders)
            ...
            total_exposure = 0.0
            positions_data = self.execution_router.get_state_metrics().get(
                "positions", {}
            )
            ...
            self._write_dashboard_state(
                equity, unrealized_pnl, current_prices, target_symbol
            )
        except Exception as e:
        ...
        for symbol in symbols_to_process:
            ...
            if df_trend is None or len(df_trend) < 20:
                df_trend = await self.data_engine.fetch_ohlcv(
                    symbol,
                    self.config.TF_TREND,
                    limit=self.config.CANDLES_ANALYSIS_LIMIT,
                )

            if df_4h is None or df_trend is None or len(df_4h) < 20:
                print(
                    ">>> DEBUG: Failed length checks:",
            ...
            if old_regime and old_regime != regime:
                logger.warning(
                    f"[{symbol}] REGIME CHANGE DETECTED: {old_regime} -> {regime}. Immediate reconciliation starting..."
                )
    ...
    def _write_dashboard_state(
        self, equity, unrealized_pnl, current_prices, target_symbol=None
    ):
        ...
        try:
            ...
            if history:
                history = sorted(
                    history, key=lambda x: str(x.get("closed_at", "")), reverse=True
                )
            ...
            state = {
                ...
                "status": self.status,
                "exchange_status": (
                    "Connected" if self.status["exchange_connected"] else "Failed"
                ),
