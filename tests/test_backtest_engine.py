...
def test_broker_sl_tp_triggering(broker):
    ...
    broker.update_on_candle({"symbol": "BTC/USDT", "open": 100, "high": 105, "low": 97, "close": 102}, 0)
    assert "BTC/USDT" in broker.positions
    
    # 2. Candle touches TP
    broker.update_on_candle({"symbol": "BTC/USDT", "open": 102, "high": 111, "low": 101, "close": 105}, 1)
...
def test_metrics_calculation():
    ...
    trades = [
        SimTrade(symbol="BTC", side="LONG", entry_price=100, exit_price=110, amount=10, pnl=100, pnl_after_fees=90, fees=10),
        SimTrade(symbol="BTC", side="LONG", entry_price=100, exit_price=95, amount=10, pnl=-50, pnl_after_fees=-55, fees=5)
    ]
