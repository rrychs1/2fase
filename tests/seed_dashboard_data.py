...
def seed_db():
    ...
    for i in range(10):
        ...
        price = 95000 + random.uniform(-2000, 2000) if "BTC" in sym else 3400 + random.uniform(-200, 200)
...
def seed_dashboard_state():
    ...
    state = {
        ...
        "history": [
            {"symbol": "BTC/USDT", "side": "LONG",  "pnl": 12.50, "closed_at": (datetime.now() - timedelta(hours=2)).isoformat()},
            {"symbol": "ETH/USDT", "side": "SHORT", "pnl": -3.20, "closed_at": (datetime.now() - timedelta(hours=1)).isoformat()},
            {"symbol": "BTC/USDT", "side": "SHORT", "pnl": 8.75,  "closed_at": datetime.now().isoformat()},
        ],
        "global_stats": {"total_trades": 10, "total_pnl": 42.35, "win_rate": 60.0, "wins": 6},
        "regimes": {"BTC/USDT": "trend", "ETH/USDT": "range"},
        "prices": {"BTC/USDT": 95120.50, "ETH/USDT": 3425.80},
        "iteration": 47,
        "uptime": "1:23:45",
        "metrics": {"signals_processed": 94, "orders_placed": 12, "orders_failed": 1, "errors": 0},
        "status": {
...
def seed_alerts():
    ...
    now = datetime.now()
    alerts = [
        {"ts": (now - timedelta(minutes=30)).isoformat(), "level": "INFO",    "msg": "Bot started in Paper mode"},
        {"ts": (now - timedelta(minutes=20)).isoformat(), "level": "TRADE",   "msg": "BTC/USDT LONG opened @ 94850.00"},
        {"ts": (now - timedelta(minutes=15)).isoformat(), "level": "WARNING", "msg": "High volatility detected on ETH/USDT"},
        {"ts": (now - timedelta(minutes=5)).isoformat(),  "level": "TRADE",   "msg": "BTC/USDT LONG closed PnL: +12.50"},
