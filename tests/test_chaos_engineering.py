...
def generate_base_window(size=50) -> pd.DataFrame:
    dates = [
        datetime.datetime.now() - datetime.timedelta(minutes=size - i)
        for i in range(size)
    ]
...
class TestChaosEngineering:
    ...
    async def test_kill_exchange_connection_randomly(self, base_config, mock_exchange):
        ...
        try:
            ...
            pytest.fail(
                f"Bot CRASHED under network partition! Unhandled exception: {e}"
            )

        assert True, "System maintained stability during aggressive network partition"

    @pytest.mark.asyncio
    async def test_websocket_data_delays_and_duplicates(
