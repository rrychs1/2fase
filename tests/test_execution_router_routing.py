...
def mock_exchange():
    ...
    exchange.fetch_order_book = AsyncMock(
        return_value={"bids": [[59900, 10]], "asks": [[60100, 10]]}
    )
...
class TestExecutionRouterRouting:
    ...
    async def test_shadow_routing_isolation(self, config, mock_exchange, signal_long):
        ...
        router.calculate_liquidity_sizing = AsyncMock(
            return_value=(signal_long.amount, signal_long.price)
        )
    ...
    async def test_live_routing_calls_engine(self, config, mock_exchange, signal_long):
        ...
        router.live_engine.place_order.assert_any_call(
            "BTC/USDT", "long", "market", 0.01
        )
    ...
    @pytest.mark.asyncio
    async def test_liquidity_scaling_applies_to_signal(
        self, config, mock_exchange, signal_long
    ):
        config.EXECUTION_MODE = "PAPER"
        router = ExecutionRouter(mock_exchange, config)
