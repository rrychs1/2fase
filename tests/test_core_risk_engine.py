...
def create_signal(symbol: str, amount: float, price: float, action: SignalAction = SignalAction.ENTER_LONG) -> Signal:
    import datetime
    return Signal(
        symbol=symbol,
        action=action,
...
class TestCoreRiskEngineUnit:
    ...
    def test_oversized_order_blocked(self, risk_engine):
        ...
        ord_oversize = create_signal("BTC/USDT", amount=150.0, price=10.0) # 1500 Notional
    ...
    def test_total_exposure_breach(self, risk_engine, base_state):
        ...
        base_state["positions"] = {
            "ETH/USDT": {"amount": 20.0, "entry_price": 100.0, "is_active": True}, # 2000 USDT
            "SOL/USDT": {"amount": 50.0, "entry_price": 50.0, "is_active": True}   # 2500 USDT
        }
    ...
    def test_max_open_positions_breach(self, risk_engine, base_state):
        ...
        for i in range(5):
            base_state["positions"][f"SYM{i}/USDT"] = {"amount": 10.0, "entry_price": 10.0, "is_active": True}
        ...
        assert risk_engine.validate_order(ord_update) is True # As long as notional passes!
...
class TestCoreRiskEngineStress:
    ...
    async def test_rapid_fire_order_splitting(self):
        ...
        state = {
            "balance": 10000.0,
            "equity": 10000.0,
            "positions": {"BTC/USDT": {"amount": 0.0, "entry_price": 10.0, "is_active": True}}
        }
