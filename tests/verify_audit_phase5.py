...
async def test_audit_phase5():
    ...
    runner.risk_manager.is_kill_switch_active = False  # Reset persistent state for test isolation
    ...
    runner.exchange.fetch_balance = AsyncMock(return_value={'total': {'USDT': 10000.0}, 'free': {'USDT': 10000.0}})
    ...
    await runner.iterate()
    assert runner.risk_manager.is_high_caution is False  # CB cleared = High Caution clears
    logger.info("BotRunner: High Caution cleared correctly after CB reset (Phase 24 behavior).")
