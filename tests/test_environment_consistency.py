...
def generate_deterministic_ohlcv(candles=300) -> pd.DataFrame:
    ...
    dates = [datetime.datetime(2025, 1, 1) + datetime.timedelta(minutes=15 * i) for i in range(candles)]
    
    # Generate a massive parabolic uptrend to force standard trend signals natively out of both engines
    close_prices = np.linspace(50000, 60000, candles) + np.random.normal(0, 100, candles)
...
async def test_environment_consistency_parity():
    ...
    async def intercepted_route(sym, regime, state):
        ...
        for s in sigs:
            if s.action in [SignalAction.ENTER_LONG, SignalAction.ENTER_SHORT, SignalAction.EXIT_LONG, SignalAction.EXIT_SHORT]:
    ...
    live_equity, _ = await bot.execution_router.get_equity_and_pnl({symbol: final_price})
    ...
    assert signal_diff <= 2, f"Logic Parity FAILED! Backtest generated {bt_signals_count} signals vs BotRunner {live_signals_count}. Discrepancy proves environment logic mismatch."
    
    # PnL difference must be constrained explicitly to realistic bounds
    pnl_diff = abs(bt_pnl - live_pnl)
    assert pnl_diff < 500.0, f"PnL validation mismatch! Difference too massive ({pnl_diff}), simulated architectures broken."
