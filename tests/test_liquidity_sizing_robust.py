...
class TestVWAPSlippageScaling:
    ...
    async def test_vwap_scaling_down(self, router, mock_exchange, base_signal, config):
        ...
        with patch.object(router.config, 'MAX_ORDER_DEPTH_RATIO', 1.0): # Ignore depth ratio to test VWAP
            # Mid = 101+99 / 2 = 100
...
class TestSpreadAndEdgeCases:
    @pytest.mark.asyncio
    async def test_max_spread_blocks_trade(self, router, mock_exchange, base_signal, config):
