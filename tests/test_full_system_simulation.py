...
class TestFullSystemEndToEndSimulation:
    ...
    async def test_30_day_full_simulation(self, sim_config, mock_exchange):
        ...
        print(
            f"[*] Frictions: Fees ({sim_config.SIMULATED_FEES_PCT*100}%), Slippage ({sim_config.SIMULATED_SLIPPAGE_BPS} bps)"
        )
        ...
        if hasattr(bot, "paper_manager") and hasattr(
            bot.paper_manager, "trade_history"
        ):
        ...
        assert (
            True
        ), "System finished successfully cleanly without any stack overflow crashes."
