...
async def main():
    ...
    try:
        ...
        for i in range(50, feed_length, 2):
            ...
            if (
                hasattr(bot.execution_router, "risk_engine")
                and bot.execution_router.risk_engine.should_shutdown()
            ):
                errors.append(
                    "System Shut Down Due to Drawdown Limit! (Expected safety trigger if severe loss)"
                )
                break
    ...
    print("\nExtracting metrics...")
    trades_taken = 0
    if hasattr(bot, "execution_router") and hasattr(
        bot.execution_router, "paper_manager"
    ):
        trades_taken = len(bot.execution_router.paper_manager.state.get("history", []))
        if trades_taken == 0:
            trades_taken = len(
                bot.execution_router.paper_manager.state.get("positions", {}).keys()
            )
    # 5. Extract results
    if hasattr(bot, "execution_router") and hasattr(
        bot.execution_router, "paper_manager"
