...
def mock_dependencies():
    with patch("orchestration.bot_runner.setup_logger"), patch(
        ...
        "orchestration.bot_runner.compute_volume_profile", return_value=MagicMock()
    ), patch(
...
def bot_config():
    ...
    config.RISK_MAX_POSITION_PER_SYMBOL = (
        0.20  # Bump limits to allow testnet entries natively
    )
    return config
...
def mock_exchange():
    ...
    exchange.fetch_order_book = AsyncMock(
        return_value={"bids": [[99.9, 10.0]], "asks": [[100.1, 10.0]]}
    )
...
def generate_bullish_pullback_data(price_start=100.0, length=50):
    ...
    df.at[length - 2, "low"] = (
        ema_fast_last - 2.5
    )  # Micro-pullback logic mapping natively
    df.at[length - 1, "close"] = (
        ema_fast_last + 25.0
    )  # Massive breakout guaranteed to trigger entry

    return df


@pytest.mark.asyncio
async def test_full_pipeline_shadow_execution(
    mock_dependencies, bot_config, mock_exchange
):
    ...
    equity, pnl = await runner.execution_router.get_equity_and_pnl(
        {"BTC/USDT": new_price}
    )
    assert pnl > 0
    ...
    with patch.object(
        runner.strategy_router, "route_signals", AsyncMock(return_value=[exit_signal])
    ):
        await runner.iterate(target_symbol="BTC/USDT")
