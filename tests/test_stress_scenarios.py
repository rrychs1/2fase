...
def mock_deps():
    ...
    for f in [
        "data/virtual_account_shadow.json",
    ...
    with patch("orchestration.bot_runner.setup_logger"), patch(
        "data.db_manager.DbManager"
    ), patch("data.websocket_manager.WebsocketManager"), patch(
        "logging_monitoring.telegram_alert_service.TelegramAlertService"
    ), patch(
        "orchestration.bot_runner.write_bot_state"
    ), patch(
        "orchestration.bot_runner.add_standard_indicators", side_effect=lambda x: x
    ), patch(
        "orchestration.bot_runner.compute_volume_profile", return_value=dummy_vp
    ), patch(
        "monitoring.metrics.bot_unrealized_pnl"
    ), patch(
        "monitoring.metrics.bot_daily_drawdown_pct"
    ), patch(
        "monitoring.metrics.bot_current_exposure"
    ), patch(
        "monitoring.metrics.bot_system_health"
    ), patch(
        "monitoring.metrics.bot_ws_connected"
    ), patch(
        "execution.shadow_executor.json.dump"
    ), patch(
        "execution.shadow_executor.open", create=True
    ):
        yield
...
async def test_flash_crash_resilience(mock_deps, stress_config):
    ...
    mock_exchange.fetch_order_book = AsyncMock(
        return_value={"bids": [[100, 1000]], "asks": [[100.1, 1000]]}
    )
    mock_exchange.validate_order_filters = MagicMock(return_value=(True, "OK"))

    ohlcv_normal = [
        [int((time.time() - (100 - i) * 3600) * 1000), 100, 101, 99, 100, 1000]
        for i in range(100)
    ]
    ohlcv_crash = [
        [int((time.time() - (100 - i) * 3600) * 1000), 100, 101, 70, 75, 1000]
        for i in range(100)
    ]

    mock_exchange.fetch_ohlcv = AsyncMock(
        side_effect=[ohlcv_normal, ohlcv_crash, ohlcv_crash, ohlcv_crash]
    )
    ...
    with patch.object(
        runner.strategy_router, "route_signals", AsyncMock(return_value=[buy_signal])
    ):
...
async def test_high_frequency_load_stress(mock_deps, stress_config):
    ...
    mock_exchange.init = AsyncMock()
    mock_exchange.fetch_balance = AsyncMock(
        return_value={"total": {"USDT": 1000000.0}, "free": {"USDT": 1000000.0}}
    )
    mock_exchange.fetch_open_orders = AsyncMock(return_value=[])
    mock_exchange.fetch_my_trades = AsyncMock(return_value=[])
    mock_exchange.fetch_order_book = AsyncMock(
        return_value={"bids": [[100, 5000]], "asks": [[101, 5000]]}
    )
    mock_exchange.validate_order_filters = MagicMock(return_value=(True, "OK"))

    ohlcv = [
        [int(time.time() * 1000) - i * 3600000, 100, 101, 99, 100, 1000]
        for i in range(100)
    ]
    ...
    with patch.object(
        runner.strategy_router, "route_signals", AsyncMock(return_value=signals)
    ):
        for i in range(10):
            await runner.iterate(target_symbol="BTC/USDT")
...
async def test_network_and_api_failure_graceful_handling(mock_deps, stress_config):
    ...
    mock_exchange.fetch_my_trades = AsyncMock(return_value=[])
    mock_exchange.fetch_order_book = AsyncMock(
        return_value={"bids": [[100, 10]], "asks": [[101, 10]]}
    )
    mock_exchange.validate_order_filters = MagicMock(return_value=(True, "OK"))
