...
def test_enforce_inventory_limits_empty_portfolio(risk_manager):
    ...
    # Max total: 5000
    current_positions = {}
    signal = Signal(symbol="BTC/USDT", action=SignalAction.ENTER_LONG, side=Side.LONG, price=50000.0, amount=0.02)
    # Notional: 50000 * 0.02 = 1000
    # Should be fully approved
    allowed = risk_manager.enforce_inventory_limits("BTC/USDT", signal, current_positions)
    assert allowed == 0.02
...
def test_enforce_inventory_limits_symbol_reduction(risk_manager):
    ...
    # Remaining = 300
    current_positions = {
        "BTC/USDT": {"amount": 0.024, "average_price": 50000.0}
    }
    
    # New signal wants 0.01 (Notional = 500)
    signal = Signal(symbol="BTC/USDT", action=SignalAction.ENTER_LONG, side=Side.LONG, price=50000.0, amount=0.01)
...
def test_enforce_inventory_limits_global_reduction(risk_manager):
    ...
    signal = Signal(symbol="ETH/USDT", action=SignalAction.ENTER_LONG, side=Side.LONG, price=300.0, amount=1.0)
    
    # Remaining globally = 200
    # Max for ETH = 1500
    # Minimum remaining = 200. Allowed amount = 200 / 300 = 0.6666...
    allowed = risk_manager.enforce_inventory_limits("ETH/USDT", signal, current_positions)
...
def test_enforce_inventory_limits_blocked(risk_manager):
    ...
    current_positions = {
        "BTC/USDT": {"amount": 0.03, "average_price": 50000.0} # 1500
    }
...
def test_enforce_inventory_limits_exit(risk_manager):
    ...
    signal = Signal(symbol="BTC/USDT", action=SignalAction.EXIT_LONG, side=Side.LONG, price=50000.0, amount=0.03)
