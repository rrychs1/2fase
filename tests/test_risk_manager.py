...
def rm(config, tmp_path):
    ...
    with patch.object(RiskManager, "DEFAULT_STATE_FILE", state_file), patch.object(
        RiskManager, "DEFAULT_LOCK_FILE", lock_file
    ):
...
class TestInventoryAndExposure:
    def test_per_asset_inventory_limit(self, rm, config, signal_long):
        ...
        # 200 / 60000 = 0.003333
        reduced_amount = rm.enforce_inventory_limits(
            "BTC/USDT", signal_long, current_positions
        )
    ...
    def test_blocked_when_at_limit(self, rm, config, signal_long):
        ...
        rm.sync_reference_equity(10000.0, 0.0)
        current_positions = {
            "ETH/USDT": {"average_price": 3000.0, "amount": 0.34}
        }  # ~10.2% (> 10%)
