...
def test_rolling_metrics_count_window():
    ...
    metrics = RollingPerformanceMetrics.calculate_rolling_metrics(
        df, window=5, min_periods=2, annualize_factor=1
    )
...
def test_rolling_metrics_time_window():
    ...
    data = {"exit_time": dates, "net_pnl": [10, 10, -10, -10]}
    df = pd.DataFrame(data)

    metrics = RollingPerformanceMetrics.calculate_rolling_metrics(
        df, window="2d", min_periods=1, annualize_factor=1
    )
