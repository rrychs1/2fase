...
def test_generate_folds_expanding():
    ...
    folds = TimeSeriesCrossValidator.generate_folds(df, n_splits=4, mode='expanding', purge_bars=2)
...
def test_generate_folds_rolling():
    df = pd.DataFrame({"close": range(100)})
    folds = TimeSeriesCrossValidator.generate_folds(df, n_splits=2, mode='rolling', purge_bars=0)
