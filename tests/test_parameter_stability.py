...
def test_generate_performance_surface():
    ...
    surface = ParameterStabilityAnalyzer.generate_performance_surface(df, mock_surface_execute, param_grid)
...
def test_stability_score():
    ...
    score_dict = ParameterStabilityAnalyzer.calculate_stability_score(df, param_cols=['fast', 'slow'], metric='sharpe_ratio', radius=1)
    ...
    assert score_dict['stability_score'] < 5.0 # Score highly penalized by variance/gradient
