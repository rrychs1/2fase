...
class StabilityTest(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        synthetic_df['timestamp'] = pd.date_range(end=datetime.now(UTC), periods=len(synthetic_df), freq='15min')
        ...
        patches = [
            patch('orchestration.bot_runner.setup_logger', return_value=self.dummy_logger),
            ...
            patch('orchestration.bot_runner.VolatilityRegimeDetector', spec=VolatilityRegimeDetector)
        ]
        for p in patches:
            p.start()
        ...
        self.runner.volatility_detector.calculate_atr = FastReturn(pd.Series([1.0]*self.config.CANDLES_ANALYSIS_LIMIT))
    ...
    async def test_long_run_stability(self):
        ...
        for i in range(num_events):
            ...
            if (i + 1) % 10000 == 0:
                ...
                print(f"[{i+1:>5}] Elapsed: {elapsed:>5.2f}s | Mem: {current_mem:>7.2f} MB | Tasks: {tasks}")
                gc.collect()
        ...
        self.assertLess(avg_latency, 2.0, "Latency too high") # Should be sub-1ms usually
