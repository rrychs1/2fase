...
def test_close_position_short():
    ...
    assert portfolio.balance == 1000.0 + 99.1 # Balance updates accurately based on net impact
    assert len(portfolio.positions) == 0
...
def test_partial_position_close():
    ...
    assert pos["fees_paid"] == pytest.approx(3.0) # 5.0 - 2.0 (proportional entry fee removed)
