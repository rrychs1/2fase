...
def main():
    ...
    check("/dashboard/health", "/dashboard/health",
          ["status", "timestamp", "can_read_bot_state", "database_ok", "database_has_data"],
          [
    ...
    # ── /api/state ───────────────────────────────────────────
    check("/api/state", "/api/state",
          ["running", "mode", "iteration", "last_loop_ts", "equity",
           "open_positions", "pending_orders", "history", "regimes",
           "prices", "last_error", "exchange_status", "has_data"],
          [
    ...
    # ── /api/metrics ─────────────────────────────────────────
    check("/api/metrics", "/api/metrics",
          ["has_data", "balance", "equity", "unrealized_pnl",
          ...
          [
              ...
              ("total_pnl is a number",   lambda d: isinstance(d["total_pnl"], (int, float))),
          ])

    # ── /api/equity-history ──────────────────────────────────
    check("/api/equity-history", "/api/equity-history",
    ...
    check("/api/status (legacy)", "/api/status",
          ["status", "is_active", "mode"],
          [("status is Running", lambda d: d["status"] == "Running")])

    check("/api/account (legacy)", "/api/account",
