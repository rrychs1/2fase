...
async def test_audit_phase3():
    ...
    exchange.fetch_balance = AsyncMock(return_value={'total': {'USDT': 10000.0}, 'free': {'USDT': 10000.0}})
    ...
    # Simulate a signal
    signal = Signal(symbol="BTC/USDT", action=SignalAction.GRID_PLACE, side=Side.LONG, price=60000.0, amount=0.01, strategy="GridInitial")
    ...
    runner.neutral_grid.grid_states["BTC/USDT"] = GridState(
        symbol="BTC/USDT",
        levels=[
            GridLevel(price=60000.0, side='buy', amount=0.01, order_id="order-123", filled=False),
            GridLevel(price=61000.0, side='buy', amount=0.01, order_id="order-456", filled=False)
        ],
        is_active=True
    )
    ...
    for l in levels:
        if l.price == 61000.0:
            ...
            logger.info(f"Reconciliation Successful: Detected missing order for level {l.price} and reset order_id")
        if l.price == 60000.0:
