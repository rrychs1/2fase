...
class DeterministicReplayTest(unittest.IsolatedAsyncioTestCase):
        ...
        df = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    end=datetime.now(UTC), periods=n_points, freq="15min"
                ),
    ...
    async def run_simulation(self, dataset):
        ...
        with patch(
            "orchestration.bot_runner.setup_logger", return_value=DummyLogger()
        ), patch("orchestration.bot_runner.write_bot_state", return_value=None), patch(
            "data.db_manager.DbManager"
        ) as mock_db_class, patch(
            "logging_monitoring.telegram_alert_service.TelegramAlertService"
        ), patch(
            "monitoring.metrics.start_metrics_exporter"
        ):
            ...
            runner = BotRunner(
                config=config, exchange=mock_exchange, data_engine=mock_data_engine
            )
            ...
            with patch(
                ...
                "orchestration.bot_runner.compute_volume_profile", return_value=vp
            ):
                ...
                for _ in range(5):
                    ...
                    current_prices = {
                        s: dataset["close"].iloc[-1] for s in config.SYMBOLS
                    }
                    eq, _ = await runner.execution_router.get_equity_and_pnl(
                        current_prices
                    )
    ...
    async def test_deterministic_replay(self):
        ...
        self.assertEqual(
            state1["balance"], state2["balance"], "Final balance mismatch!"
        )
        self.assertEqual(state1["metrics"], state2["metrics"], "Metrics mismatch!")
        self.assertEqual(
            state1["positions"], state2["positions"], "Positions mismatch!"
        )
