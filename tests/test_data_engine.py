...
async def test_update_ohlcv_deduplication_and_sorting(data_engine):
    ...
    initial_df = pd.DataFrame(
        [
            {
                "timestamp": pd.to_datetime(1000, unit="ms"),
                "open": 10,
                "high": 15,
                "low": 9,
                "close": 12,
            ...
            {
                "timestamp": pd.to_datetime(2000, unit="ms"),
                "open": 12,
                "high": 18,
                "low": 11,
                "close": 15,
...
async def test_rolling_window_limit(data_engine, mock_exchange):
    ...
    initial_data = [
        {
            "timestamp": pd.to_datetime(i * 1000, unit="ms"),
            "open": 10,
            "high": 15,
            "low": 9,
            "close": 10 + i,
    ...
    assert len(df) == 5
    assert df.iloc[0]["timestamp"] == pd.to_datetime(
        1000, unit="ms"
    )  # 0th (1000ms) dropped, 1st remains
