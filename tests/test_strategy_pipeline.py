...
class TestStrategyPipelineQuantitativeAudit:
    ...
    def test_pipeline_rejects_overfitted_strategy(self, mock_load):
        ...
        ), "A massive Out-Of-Sample bleeder was erroneously validated!"
        assert result["sharpe"] < 0.5, "Overfitted WFA Sharpe incorrectly padded"
        assert (
            result["stability_score"] < 50.0
        ), "Overfitted Strategy Stability Score incorrectly high"
        assert (
            "Negative Aggregated WFA" in result["reason"]
            or "Dangerous Aggregated WFA" in result["reason"]
        ), "Failed to identify structural PnL damage"
    ...
    def test_pipeline_accepts_stable_strategy(self, mock_load):
        ...
        ), f"Stable Strategy Unjustly Rejected: {result['reason']}"
        assert result["sharpe"] >= 0.5, f"Stable WFA Sharpe too low: {result['sharpe']}"
        assert (
            result["stability_score"] > 50.0
        ), "Stability Score did not appropriately reward robust tracking metrics"
    ...
    def test_pipeline_catches_drawdown_drift(self, mock_load):
        ...
        assert (
            result["is_valid"] is False
        ), "A massive localized WFA Drawdown was allowed into production blindly!"
        assert (
            "Dangerous Aggregated WFA Drawdown" in result["reason"]
        ), "Failed to identify the sharp Volatility spike explicitly."
