...
@pytest.fixture
def broker():
    return SimBroker(initial_balance=10000.0, maker_fee=0.0004, taker_fee=0.0006, slippage=0.0001)


class TestOpenPosition:
    def test_open_long_deducts_fee(self, broker):
        sig = Signal(symbol="BTC/USDT", action=SignalAction.ENTER_LONG, side=Side.LONG,
                     price=60000.0, amount=0.1, stop_loss=58000.0, take_profit=64000.0)
    ...
    def test_open_short(self, broker):
        sig = Signal(symbol="BTC/USDT", action=SignalAction.ENTER_SHORT, side=Side.SHORT,
                     price=60000.0, amount=0.1)
        broker.process_signal(sig, current_price=60000.0)
    ...
    def test_no_duplicate_position(self, broker):
        ...
        assert broker.positions["BTC/USDT"].entry_price == pytest.approx(60000 * 1.0001, rel=1e-3)

    def test_zero_amount_ignored(self, broker):
        sig = Signal(symbol="BTC/USDT", action=SignalAction.ENTER_LONG, side=Side.LONG,
                     price=60000.0, amount=0.0)
        broker.process_signal(sig, current_price=60000.0)
...
class TestClosePosition:
    def test_close_long_profit(self, broker):
        ...
        close_sig = Signal(symbol="BTC/USDT", action=SignalAction.EXIT_LONG, side=Side.LONG,
                          price=62000.0)
...
class TestStopLossAndTakeProfit:
    def test_sl_triggers_long(self, broker):
        ...
        broker.process_signal(sig, current_price=60000.0)
        # Candle hits SL
        broker.update_on_candle({"symbol": "BTC/USDT", "open": 59000, "high": 59500, "low": 57500, "close": 58500})
        assert "BTC/USDT" not in broker.positions
        assert len(broker.trades) == 1
    ...
    def test_tp_triggers_long(self, broker):
        ...
        broker.process_signal(sig, current_price=60000.0)
        # Candle hits TP
        broker.update_on_candle({"symbol": "BTC/USDT", "open": 63000, "high": 65000, "low": 62000, "close": 64500})
        assert "BTC/USDT" not in broker.positions
        assert broker.trades[0].pnl > 0
    ...
    def test_none_sl_tp_no_crash(self, broker):
        ...
        broker.update_on_candle({"symbol": "BTC/USDT", "open": 50000, "high": 70000, "low": 50000, "close": 60000})
        assert "BTC/USDT" in broker.positions
...
class TestPendingOrders:
    def test_grid_order_fills_on_price_cross(self, broker):
        ...
        sig = Signal(symbol="BTC/USDT", action=SignalAction.GRID_PLACE, side=Side.LONG,
                     price=59000.0, amount=0.05)
        broker.process_signal(sig, current_price=60000.0)
        assert len(broker.pending_orders) == 1
        # Candle touches 59000
        broker.update_on_candle({"symbol": "BTC/USDT", "open": 60000, "high": 60000, "low": 58500, "close": 59500})
        assert len(broker.pending_orders) == 0
        assert "BTC/USDT" in broker.positions

    def test_grid_order_no_fill_if_price_doesnt_cross(self, broker):
        sig = Signal(symbol="BTC/USDT", action=SignalAction.GRID_PLACE, side=Side.LONG,
                     price=55000.0, amount=0.05)
        broker.process_signal(sig, current_price=60000.0)
        broker.update_on_candle({"symbol": "BTC/USDT", "open": 60000, "high": 61000, "low": 59000, "close": 60500})
        assert len(broker.pending_orders) == 1
