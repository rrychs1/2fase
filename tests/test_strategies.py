...
def test_grid_level_generation_volatility_scaling(grid_strategy):
    ...
    levels_high = grid_strategy.generate_grid_levels(
        "BTC/USDT", vp, 1000.0, market_state
    )
...
async def test_grid_rebuild_after_cooldown(grid_strategy):
    ...
    with patch("time.time", return_value=1700.0):
        signals = await grid_strategy.on_market_state(
            "BTC/USDT", market_state
        )  # outside (3) + cooldown ok
        assert len(signals) > 0
...
async def test_trend_dca_replenishment(trend_strategy):
    ...
    trend_strategy.active_positions["BTC/USDT"] = MagicMock(
        is_active=True, dca_levels=[MagicMock(price=90.0, amount=1.0, filled=False)]
    )
...
async def test_router_delegation(grid_strategy, trend_strategy):
    ...
    # 1. Range regime
    grid_strategy.on_market_state = AsyncMock(
        return_value=[Signal("BTC", SignalAction.HOLD)]
    )
    ...
    # 2. Trend regime
    trend_strategy.on_new_candle = AsyncMock(
        return_value=[Signal("BTC", SignalAction.ENTER_LONG)]
    )
