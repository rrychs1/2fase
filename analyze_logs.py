...
def analyze_logs():
        ...
        with open(LOG_FILE, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                if "FILLED" in line:
                    ...
                    match = re.search(
                        r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*\[Grid\] (\w+/\w+) Level (\d+\.?\d*) \((\w+)\) FILLED",
                        line,
                    )
                    if match:
    ...
    if not trades:
        ...
        for t in trades:
            print(
                f"{t['time']} | {t['symbol']:<10} | {t['side']:<6} | {t['price']:.2f}"
            )
    ...
    if len(trades) > 0:
        ...
        if stops > 0:
            print(
                f"- ⚠️ SE ACTIVÓ EL KILL SWITCH {stops} VECES. Esto indica pérdidas controladas o errores de saldo."
            )
        else:
            print(
                "- ✅ No se han registrado paradas de emergencia (Kill Switch) por pérdidas masivas en este periodo analizado (filtrando errores de saldo 0)."
            )

        print(
            "- La estrategia de Grid parece estar funcionando para capturar movimientos pequeños."
        )
    else:
        print(
            "- No hay suficiente actividad de trading registrada para evaluar rentabilidad."
        )
        print(
            "- La mayoría de los logs parecen ser de monitoreo o errores de conexión."
        )
