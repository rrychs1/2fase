...
def calculate_metrics(
    ...
    if len(equity) > 1:
        ...
        if len(returns) > 0 and np.std(returns) > 0:
            ...
            sharpe_ratio = (np.mean(returns) / np.std(returns)) * np.sqrt(candles_per_year)
        else:
            sharpe_ratio = 0.0
    else:
        sharpe_ratio = 0.0
    ...
    if len(equity) > 1:
        ...
        annual_return = (total_return_pct / 100) * (365 / trading_days) if trading_days > 0 else 0.0
        calmar_ratio = annual_return / (max_drawdown_pct / 100) if max_drawdown_pct > 0 else 0.0
    else:
        calmar_ratio = 0.0

    # --- Buy & Hold ---
    buy_and_hold_return_pct = ((buy_hold_end_price - buy_hold_start_price) / buy_hold_start_price) * 100
...
def print_metrics(metrics: BacktestMetrics):
    ...
    print(f"  Max Drawdown:         {metrics.max_drawdown_pct:>11.2f}%  (${metrics.max_drawdown_abs:,.2f})")
