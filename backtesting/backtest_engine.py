...
class BacktestEngine:
    ...
    def run(self, df: pd.DataFrame) -> BacktestMetrics:
        ...
        """
        logger.info(f"Starting backtest: {self.symbol} | {len(df)} candles | "
        ...
        if start_idx >= total_candles:
            logger.error(f"Not enough candles ({total_candles}) for lookback ({self.lookback})")
        ...
        for i in range(start_idx, total_candles):
            ...
            equity = self.broker.get_equity_with_unrealized({self.symbol: current_price})
            ...
            if i == start_idx or (i - start_idx) % max(1, (total_candles - start_idx) // 10) == 0:
                logger.info(
        ...
        self.broker.force_close_all({self.symbol: last_price}, candle_idx=total_candles - 1)

        # Final equity after closing
        final_eq = self.broker.balance
        self.broker.equity_curve[-1] = final_eq

        logger.info(f"Backtest complete. {signals_generated} signals, {len(self.broker.trades)} trades.")
    ...
    def _empty_metrics(self, df) -> BacktestMetrics:
        ...
        return BacktestMetrics(
            total_return_pct=0, total_pnl=0, sharpe_ratio=0,
            max_drawdown_pct=0, max_drawdown_abs=0, win_rate=0,
            profit_factor=0, total_trades=0, winning_trades=0,
            losing_trades=0, avg_trade_pnl=0, avg_winner=0,
            avg_loser=0, max_consecutive_losses=0, calmar_ratio=0,
            buy_and_hold_return_pct=0, initial_equity=self.broker.initial_balance,
            final_equity=self.broker.initial_balance, peak_equity=self.broker.initial_balance,
        )
