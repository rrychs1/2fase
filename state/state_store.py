...
class StateStore:
    ...
    def save_position(self, symbol: str, amount: float, entry_price: float):
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("INSERT OR REPLACE INTO positions (symbol, amount, entry_price) VALUES (?, ?, ?)", 
    ...
    def load_positions(self):
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {row[0]: {"amount": row[1], "entry_price": row[2]} for row in c.fetchall()}

    def get_position(self, symbol: str):
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                c = conn.cursor()
                c.execute("SELECT amount, entry_price FROM positions WHERE symbol = ?", (symbol,))
                row = c.fetchone()
                if row:
    ...
    def save_balance(self, balance: float):
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("INSERT OR REPLACE INTO balance (id, balance) VALUES (1, ?)", (balance,))
    ...
    def get_open_orders(self):
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                c = conn.cursor()
                c.execute("SELECT order_id, status FROM orders WHERE status != 'FILLED' AND status != 'FAILED'")
