...
class TrendDcaStrategy:
    ...
    def generate_trend_signal(self, symbol: str, df: pd.DataFrame) -> Side:
        ...
        is_bullish = (
            last_row["EMA_fast"] > last_row["EMA_slow"] and last_row["MACD"] > 0
        )
        is_bearish = (
            last_row["EMA_fast"] < last_row["EMA_slow"] and last_row["MACD"] < 0
        )
        ...
        logger.debug(
            f"[TrendDca] {symbol} Trend Check: EMA_f={last_row['EMA_fast']:.2f}, EMA_s={last_row['EMA_slow']:.2f}, MACD={last_row['MACD']:.4f}"
        )
    ...
    def plan_dca_levels(
        self, entry_price: float, side: Side, total_amount: float
    ) -> list[DcaLevel]:
        """
    ...
    async def on_new_candle(self, symbol: str, market_state: dict) -> list[Signal]:
        ...
        total_amount = (
            equity * 0.15
        )  # 15% allocation for trend + DCA (conservative for testnet)
        ...
        if not position_state or not position_state.get("is_active", False):
            ...
            if trend == Side.LONG:
                ...
                is_pullback = (
                    current_price <= recent_high * (1 - pullback_tolerance)
                    and current_price > last_row["EMA_slow"]
                )
                ...
                if is_pullback or is_breakout:
                    ...
                    logger.info(
                        f"[TrendDca] {symbol} {reason} LONG detected at {current_price}"
                    )
                    sl, tp = self.calculate_sl_tp(current_price, Side.LONG, atr)

                    dca_levels = self.plan_dca_levels(
                        current_price, Side.LONG, total_amount
                    )
                    ...
                    logger.info(
                        f"[TrendDca] {symbol} Bullish trend. Waiting for {pullback_tolerance*100}% pullback or breakout."
                    )

            elif trend == Side.SHORT:
                # SHORT Pullback (1.5% from recent low) or Breakout
                is_pullback = (
                    current_price >= recent_low * (1 + pullback_tolerance)
                    and current_price < last_row["EMA_slow"]
                )
                ...
                if is_pullback or is_breakout:
                    reason = "Pullback" if is_pullback else "Breakout"
                    logger.info(
                        f"[TrendDca] {symbol} {reason} SHORT detected at {current_price}"
                    )
                    sl, tp = self.calculate_sl_tp(current_price, Side.SHORT, atr)

                    dca_levels = self.plan_dca_levels(
                        current_price, Side.SHORT, total_amount
                    )
                    ...
                    logger.info(
                        f"[TrendDca] {symbol} Bearish trend. Waiting for {pullback_tolerance*100}% pullback or breakout."
                    )
            else:
        ...
        # 2. Logic for EXISTING Positions (Exit & DCA)
            ...
            if (
                ...
                or not self.active_positions[symbol].is_active
            ):
            ...
            if side == "LONG":
                ...
                for level in dca_levels:
                    if not level.filled and current_price <= level.price:
                        logger.info(
                            f"[TrendDca] {symbol} LONG DCA Level {level.price} hit!"
                        )
                        signals.append(
                            Signal(
                                symbol=symbol,
                ...
                for level in dca_levels:
                    if not level.filled and current_price >= level.price:
                        logger.info(
                            f"[TrendDca] {symbol} SHORT DCA Level {level.price} hit!"
                        )
                        signals.append(
                            Signal(
                                symbol=symbol,
    ...
    def reconcile_with_exchange(self, symbol: str, open_orders: list[dict]):
        ...
        for level in pos.dca_levels:
            if level.order_id and level.order_id not in order_ids_on_exchange:
                if not level.filled:
                    logger.warning(
                        f"[TrendDca] {symbol} DCA Order {level.order_id} missing from exchange. Resetting."
                    )
                    level.order_id = None
