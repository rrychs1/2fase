...
class ParameterStabilityAnalyzer:
    ...
    @staticmethod
    def generate_performance_surface(
        df: pd.DataFrame,
        execute_func: Callable[[pd.DataFrame, dict], dict],
    ...
    def calculate_stability_score(
        ...
        for p, p_idx_col in grid_coords.items():
            ...
            neighbor_mask &= (df_eval[p_idx_col] >= target_idx - radius) & (
                df_eval[p_idx_col] <= target_idx + radius
            )
        ...
        if len(neighbor_vals) <= 1:
            ...
            stability_score = float(
                best_val / (1.0 + local_gradient + neighborhood_variance)
            )
    ...
    @staticmethod
    def slice_surface(
        surface_df: pd.DataFrame, fixed_params: Dict[str, Any]
    ) -> pd.DataFrame:
        """
