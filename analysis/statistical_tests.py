...
class StatisticalOverfittingTests:
    ...
    @staticmethod
    def generate_block_bootstrap(returns: np.ndarray, block_size: int = 5, n_bootstraps: int = 10000) -> np.ndarray:
        """
        ...
        random_starts = np.random.randint(0, valid_starts, size=(n_bootstraps, n_blocks))
    ...
    def calculate_deflated_sharpe_ratio(
        ...
        kurtosis = float(stats.kurtosis(returns, fisher=False)) # Pearson's (normal = 3)
    ...
    def hansens_spa(
        ...
        bootstrapped_d = StatisticalOverfittingTests.generate_block_bootstrap(d_k, block_size, n_bootstraps)
    ...
    @staticmethod
    def calculate_p_value_bootstrap(returns: np.ndarray, n_bootstraps: int = 10000, block_size: int = 5) -> float:
        """
        ...
        bootstrapped = StatisticalOverfittingTests.generate_block_bootstrap(centered_returns, block_size, n_bootstraps)
        
        boot_means = np.mean(bootstrapped, axis=1)
        return float(np.mean(boot_means >= obs_mean))

    @staticmethod
    def calculate_overfitting_risk_score(dsr_p: float, spa_p: float, boot_p: float) -> float:
        """
