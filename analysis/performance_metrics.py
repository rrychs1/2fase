...
class PerformanceMetrics:
    ...
    @staticmethod
    def calculate_equity_metrics(equity_series: pd.Series, risk_free_rate: float = 0.0, annualization_factor: int = 365) -> Dict[str, float]:
        """
        ...
        if days_elapsed > 0:
            annualized_return = (1 + total_return) ** (annualization_factor / days_elapsed) - 1.0
        else:
        ...
        if daily_volatility > 0:
            sharpe_ratio = (mean_return - risk_free_rate) / daily_volatility * np.sqrt(annualization_factor)
        else:
            sharpe_ratio = 0.0
        ...
        if downside_std > 0:
            sortino_ratio = (mean_return - risk_free_rate) / downside_std * np.sqrt(annualization_factor)
        else:
    ...
    @staticmethod
    def calculate_trade_metrics(trades: Union[List[dict], pd.DataFrame]) -> Dict[str, float]:
        """
    ...
    @staticmethod
    def calculate_full_tearsheet(equity_series: pd.Series, trades: Union[List[dict], pd.DataFrame], initial_equity: float = 10000.0) -> dict:
        """
        ...
        if isinstance(equity_series, pd.Series) and len(equity_series) > 1:
            ...
            if not returns.empty:
                curves = RiskMetrics.run_monte_carlo_simulation(returns.values, initial_equity=initial_equity)
                mc_stats = RiskMetrics.calculate_monte_carlo_statistics(curves, initial_equity=initial_equity)
