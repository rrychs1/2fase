    ...
    def calculate_rolling_metrics(
        ...
        if isinstance(window, str):
            ...
            if 'exit_time' in df.columns:
                ...
                raise ValueError("String window requires 'exit_time' column for time index.")
        ...
        metrics['rolling_win_rate'] = is_win.rolling(window=window, min_periods=min_periods).mean()
        
        # 4. Rolling Profit Factor
        pos_pnl = df['net_pnl'].where(df['net_pnl'] > 0, 0)
        neg_pnl = df['net_pnl'].where(df['net_pnl'] < 0, 0).abs()
        
        roll_gross_profit = pos_pnl.rolling(window=window, min_periods=min_periods).sum()
        ...
        avg_win = df['net_pnl'][df['net_pnl'] > 0].rolling(window=window, min_periods=min_periods).mean()
        avg_loss = df['net_pnl'][df['net_pnl'] < 0].abs().rolling(window=window, min_periods=min_periods).mean()
        ...
        metrics['current_drawdown_pnl'] = metrics['cummax_pnl'] - metrics['cumulative_pnl']
