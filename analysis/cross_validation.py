...
class TimeSeriesCrossValidator:
    ...
    @staticmethod
    def generate_folds(
        df: pd.DataFrame, n_splits: int, mode: str = "expanding", purge_bars: int = 0
    ...
    def aggregate_metrics(
        tearsheets: List[Dict], weighting: str = "trades"
    ) -> Dict[str, float]:
        """
        ...
        if "total_return_pct" in df.columns:
            ...
            aggregated["profitable_folds_pct"] = float(
                (profitable_folds / len(df)) * 100.0
            )
        ...
        for train_df, test_df in folds:
            try:
                ...
                if trades:
                    ...
                    tearsheets.append(
                        StrategyEvaluator.evaluate_strategy(pd.DataFrame())
                    )
