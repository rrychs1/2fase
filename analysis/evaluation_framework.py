...
class StrategyEvaluator:
    ...
    def generate_synthetic_equity_curve(
        trades_df: pd.DataFrame, initial_balance: float = 10000.0
    ) -> pd.Series:
        """
    ...
    def evaluate_strategy(
        ...
        """
        equity_curve = StrategyEvaluator.generate_synthetic_equity_curve(
            trades_df, initial_balance
        )

        tearsheet = PerformanceMetrics.calculate_full_tearsheet(
            equity_series=equity_curve, trades=trades_df, initial_equity=initial_balance
        )
        ...
        if len(equity_curve) > 5:
            ...
            spa = StatisticalOverfittingTests.hansens_spa(
                returns, n_bootstraps=5000, block_size=5
            )
            boot_p = StatisticalOverfittingTests.calculate_p_value_bootstrap(
                returns, n_bootstraps=5000, block_size=5
            )
    ...
    def generate_markdown_report(
        tearsheet: dict, strategy_name: str = "Strategy", filepath: str = None
    ) -> str:
        """
        ...
        md += (
            f"- **Block Bootstrap p-value:** {tearsheet.get('boot_p_value', 1.0):.4f}\n"
        )
