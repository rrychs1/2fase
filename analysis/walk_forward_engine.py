...
class WalkForwardValidator:
    ...
    def generate_windows(
        df: pd.DataFrame, train_size: int, test_size: int, step_size: int
    ...
    def run_walk_forward(
        ...
        windows = WalkForwardValidator.generate_windows(
            df, train_size, test_size, step_size
        )
        if not windows:
            return [], {}
        ...
        for idx, (train_df, test_df) in enumerate(windows):
            logger.info(
                f"--- Processing Walk-Forward Window {idx + 1}/{len(windows)} ---"
            )

            # 1. Optimize on IN-SAMPLE (Train) Data
            best_params = WalkForwardValidator.optimize_parameters(
                train_df, eval_func, param_grid
            )
            ...
            try:
                ...
                reports.append(
                    {
                        ...
                        "train_start": (
                            ...
                            else train_df.iloc[0].name
                        ),
                        "train_end": (
                            train_df.index[-1]
                            if isinstance(train_df.index, pd.DatetimeIndex)
                            else train_df.iloc[-1].name
                        ),
                        "test_start": (
                            test_df.index[0]
                            if isinstance(test_df.index, pd.DatetimeIndex)
                            else test_df.iloc[0].name
                        ),
                        "test_end": (
                            test_df.index[-1]
                            if isinstance(test_df.index, pd.DatetimeIndex)
                            else test_df.iloc[-1].name
                        ),
                        "params": best_params,
                ...
                logger.error(
                    f"Out-of-sample execution failed for window {idx + 1}: {e}"
                )
        ...
        tearsheet = StrategyEvaluator.evaluate_strategy(
            trades_df, initial_balance=10000.0
        )
