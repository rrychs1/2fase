...
class StrategyDecayMonitor:
    ...
    def register_strategy(
        self, strategy_id: str, config_overrides: dict = None
    ) -> None:
        """
    ...
    def update_trades(self, strategy_id: str, trades_df: pd.DataFrame) -> None:
        ...
        if strategy_id not in self.strategies:
            logger.warning(
                f"Unregistered strategy: {strategy_id}. Auto-registering with default config."
            )
    ...
    def evaluate_strategy_health(self, strategy_id: str) -> dict:
        ...
        if health_score < cfg["health_critical_threshold"]:
            alerts.append(
                {
                    "type": "CRITICAL",
                    "message": f"Health Score below CRITICAL threshold ({health_score:.1f}/100)",
                }
            )
            ...
            alerts.append(
                {
                    "type": "WARNING",
                    "message": f"Health Score below WARNING threshold ({health_score:.1f}/100)",
                }
            )
