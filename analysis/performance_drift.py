...
class PerformanceDriftDetector:
    ...
    def calculate_drift(
        ...
        rolling_df = RollingPerformanceMetrics.calculate_rolling_metrics(
            trades_df, window=granular_window, min_periods=5
        )
        ...
        if (
            ...
            and len(rolling_df) > historical_baseline_window + recent_window
        ):
            history_block = rolling_df.iloc[
                -(historical_baseline_window + recent_window) : -recent_window
            ]
        else:
        ...
        for metric, weight in weights.items():
            if (
                metric not in history_block.columns
                or history_block[metric].isna().all()
            ):
                continue
