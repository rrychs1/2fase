...
class StrategyValidationPipeline:
    ...
    def _eval_func(self, df: pd.DataFrame, params: dict) -> float:
        ...
        engine = BacktestEngine(
            symbol=self.symbol, timeframe=self.timeframe, lookback=250
        )
    ...
    def run_all(self, strategy_name: str) -> Dict:
        ...
        """
        logger.info(
            f"[Validation Pipeline] Starting Walk-Forward Analysis (WFA) for {strategy_name} on {self.symbol}"
        )

        # Load 1 full year of data for robust chronological window rolling
        start_dt = (datetime.datetime.now() - datetime.timedelta(days=360)).strftime(
            "%Y-%m-%d"
        )
        ...
        try:
            df = load_historical(
                self.symbol, self.timeframe, start_date=start_dt, end_date=end_dt
            )
            if df.empty or len(df) < 500:
                raise ValueError("Dataset empty")
        except Exception:
            df = load_historical(
                self.symbol, self.timeframe, "2025-01-01", "2026-01-01"
            )

        if (
            df.empty or len(df) < 500
        ):  # We need minimum rows safely pad the lookback logic
            logger.error("[Validation Pipeline] Insufficient data.")
            return {
                "is_valid": False,
                "reason": "Insufficient offline data for WFA chronological validation",
            }
        ...
        logger.info(
            f"[Validation Pipeline] WFA Splitting array into windows (Train: {train_size//CPD}d, Test: {test_size//CPD}d)..."
        )
        ...
        if (
            wfa_drawdown < -15.0
        ):  # Strategy explicitly structurally fails the Hard block at 15% unified OOS drawdown
            return {
                "is_valid": False,
        ...
        logger.info(
            f"[Validation Pipeline] {strategy_name} PASSED WFA! Sharpe: {wfa_sharpe:.2f}, Stability: {stability_score:.1f}%"
        )
        return {
            "is_valid": True,
