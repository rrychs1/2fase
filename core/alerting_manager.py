...
class AlertManager:
    ...
    def __init__(self, history_file: str = "logs/alert_history.jsonl"):
        ...
        self.last_alerts: Dict[str, Dict[str, Any]] = (
            {}
        )  # {alert_hash: {time: timestamp, count: int}}
    ...
    async def send_alert(
        ...
        # Update state
        self.last_alerts[alert_hash] = {"time": time.time(), "last_message": message}
        ...
        try:
            ...
            async with session.post(url, json={"content": content}) as response:
                if response.status >= 400:
                    logger.error(
                        f"Webhook dispatch failed with status {response.status}"
                    )
        except Exception as e:
    ...
    async def _dispatch_email(self, payload: Dict):
        ...
        try:
            ...
            msg["Subject"] = str(
                f"TRADING BOT ALERT: {payload['severity']} - {payload['strategy_id']}"
            )
