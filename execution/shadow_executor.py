...
class ShadowExecutor:
    ...
    def execute_signal(self, signal: Signal):
        ...
        if signal.action.value in [
            SignalAction.ENTER_LONG.value,
            SignalAction.ENTER_SHORT.value,
        ]:
            ...
            side = (
                "LONG"
                if signal.action.value == SignalAction.ENTER_LONG.value
                else "SHORT"
            )
            ...
            logger.info(
                f"[SHADOW] Executed {side} on {symbol} at {signal.price} (Size: {signal.amount})"
            )
            ...
            if symbol in self.state["positions"]:
                ...
                # New Average Price
                pos["average_price"] = (
                    (pos["average_price"] * old_total) + (signal.price * new_add)
                ) / (old_total + new_add)
                pos["amount"] += new_add
                logger.info(
                    f"[SHADOW] {symbol} DCA filled at {signal.price}. New Avg: {pos['average_price']:.2f} Size: {pos['amount']:.4f}"
                )
            ...
            logger.info(
                f"[SHADOW] Grid order simulated at {signal.price} (Not tracked pending in Shadow mode yet)"
            )

        elif signal.action.value in [
            SignalAction.EXIT_LONG.value,
            SignalAction.EXIT_SHORT.value,
        ]:
            if symbol in self.state["positions"]:
                ...
                logger.info(
                    f"[SHADOW] Closed {pos['side']} on {symbol} at {signal.price}. PnL: {pnl:.2f}"
                )
        ...
        return {
            "id": order_id,
            "status": "closed",
            "info": {"simulated": True, "mode": "shadow"},
        }
