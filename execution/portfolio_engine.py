...
class Portfolio:
    ...
    def simulate_execution(
        self, price: float, side: str, is_maker: bool = False
    ) -> tuple[float, float]:
        """
        ...
        if not is_maker:
            ...
            executed_price = (
                price + slippage if side.upper() == "LONG" else price - slippage
            )
        else:
    ...
    def open_position(
        ...
        if symbol in self.positions:
            ...
            if pos["side"] != side:
                raise ValueError(
                    f"Hedging not supported in simple portfolio. Active side: {pos['side']}"
                )
            ...
            logger.debug(
                f"Added to {symbol} position. New Avg: {pos['average_price']}, Amount: {pos['amount']}"
            )
        else:
    ...
    def update_price(self, symbol: str, price: float):
        ...
        if symbol in self.positions:
            ...
            pnl = (
                diff * pos["amount"] if pos["side"] == "LONG" else -diff * pos["amount"]
            )
            pos["unrealized_pnl"] = pnl

    def close_position(
        self, symbol: str, price: float, amount: float = None, is_maker: bool = False
    ) -> Optional[dict]:
        """
        ...
        if close_amount > pos["amount"]:
            logger.warning(
                f"Requested close amount {close_amount} > active {pos['amount']}. Closing all."
            )
        ...
        gross_pnl = (
            diff * close_amount if pos["side"] == "LONG" else -diff * close_amount
        )

        # Calculate proportional entry fees for the amount being closed
        proportional_entry_fees = (
            (pos["fees_paid"] / pos["amount"]) * close_amount
            if pos["amount"] > 0
            else 0
        )
        ...
        self.balance += (
            gross_pnl  # Add the gross PnL to balance, fees are subtracted separately
        )
        ...
        if close_amount >= pos["amount"]:
            ...
            pos[
                "fees_paid"
            ] -= proportional_entry_fees  # Remaining fees stay with remaining amount
        ...
        logger.debug(
            f"Closed {close_amount} of {pos['side']} on {symbol}. Net PnL: {net_pnl:.2f}"
        )
        return trade_record
