...
class ExecutionRouter:
    ...
    async def calculate_liquidity_sizing(
        self, symbol: str, signal: Signal
    ) -> tuple[float, float]:
        """
        ...
        try:
            ...
            if not ob or ob.get("bids") is None or ob.get("asks") is None:
                logger.warning(
                    f"[Liquidity] Missing OB for {symbol}. Allowing size but flagging warning."
                )
            ...
            if not bids or not asks:
                logger.warning(
                    f"[Liquidity] One side of OB for {symbol} is empty. Blocking."
                )
                return 0.0, mid_fallback
            ...
            if spread_pct > max_spread:
                logger.warning(
                    f"[Liquidity] Spread {spread_pct*100:.2f}% > Max {max_spread*100:.2f}%. Blocking."
                )
            ...
            levels = (
                asks
                if signal.side == Side.LONG or signal.action == SignalAction.EXIT_SHORT
                else bids
            )
            limit_price = mid_price * 1.01 if levels is asks else mid_price * 0.99

            for price, qty in levels:
                if (levels is asks and price > limit_price) or (
                    levels is bids and price < limit_price
                ):
                    break
                depth_vol += qty
            ...
            if target_amount < amount:
                logger.warning(
                    f"[Liquidity] {symbol} Order > 10% Depth. Reduced {amount:.4f} -> {target_amount:.4f}"
                )
            ...
            if cumulative_vol > 0:
                ...
                if slippage > max_slip:
                    logger.warning(
                        f"[Liquidity] {symbol} VWAP Slippage {slippage*100:.2f}% > Max {max_slip*100:.2f}%."
                    )
                    ...
                    logger.warning(
                        f"[Liquidity] Auto-scaled size to {target_amount:.4f} to fit slippage tolerance."
                    )
    ...
    async def execute_signal(self, signal: Signal, neutral_grid=None, trend_dca=None):
        ...
        if self.risk_engine.should_shutdown():
            logger.critical(
                "[Router] HARD KILL SWITCH TRIGGERED: Global risk limits breached. Cancelling all orders & closing positions."
            )
        ...
        if not self.risk_engine.validate_order(signal):
            logger.error(
                f"[Router] Order for {symbol} rejected by CoreRiskEngine constraints."
            )
            return None

        # Apply Advanced Liquidity Sizing globally across all modes
        original_amount = signal.amount
        allowed_amount, mid_price = await self.calculate_liquidity_sizing(
            symbol, signal
        )
        ...
        if self.mode == "SHADOW":
            ...
            if signal.action == SignalAction.GRID_PLACE:
                order_res = await self.live_engine.place_order(
                    symbol,
                ...
                order_res = await self.live_engine.place_order(
                    symbol, signal.side.name.lower(), "market", signal.amount
                )
                ...
                if order_res and order_res.get("id"):
                    if getattr(signal, "stop_loss", None):
                        ...
                        await self.live_engine.place_order(
                            symbol,
                            sl_side,
                            "stop",
                            signal.amount,
                    ...
                    for dca_m in dca_levels_meta:
                        dca_order = await self.live_engine.place_order(
                            symbol,
                            signal.side.name.lower(),
                            "limit",
                            dca_m["amount"],
                        ...
                        if dca_order and dca_order.get("id") and trend_dca:
                            trend_dca.update_order_id(
                                symbol, dca_m["price"], dca_order["id"]
                            )

            elif signal.action in [SignalAction.EXIT_LONG, SignalAction.EXIT_SHORT]:
                order_res = await self.live_engine.place_order(
                    symbol,
                    "sell" if signal.side == Side.LONG else "buy",
                    "market",
                    signal.amount,
                    params={"reduceOnly": True},
                )
