...
class ExecutionEngine:
    ...
    async def execute_order_safe(self, signal, order_type: str, params: dict = None) -> dict:
        """
        ...
        if not self.tracker.register(order_id):
            if self.tracker.orders.get(order_id) != OrderState.FAILED:
                logger.warning(f"[Idempotency] Order {order_id} ({signal.symbol}) already executing/executed. Blocking duplicate.")
                return None
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                side_str = signal.side.name.lower() if hasattr(signal.side, 'name') else str(signal.side).lower()
                ...
                if status == 'closed' or status == 'filled':
                    ...
                    logger.info(f"Order FILLED natively", extra={"event": "OrderFilled", "symbol": signal.symbol, "latency_ms": latency_ms, "pnl": 0.0, "amount": signal.amount})
                elif status == 'open':
                    self.tracker.update_status(order_id, OrderState.SENT)
                    logger.info(f"Order ROUTED natively", extra={"event": "OrderSent", "symbol": signal.symbol, "latency_ms": latency_ms})
                else:
                ...
                err_str = str(e).lower()
                is_timeout = 'timeout' in err_str or '502' in err_str or 'network' in err_str
                
                if is_timeout and attempt < max_retries:
                    logger.warning(f"[Retry] Network/Timeout error on {order_id}. Retrying {attempt+1}/{max_retries}...", extra={"event": "OrderRetry", "symbol": signal.symbol})
                    self.tracker.increment_retry(order_id)
                    await asyncio.sleep(base_backoff_sec * (2 ** attempt))
                    continue
                else:
                    self.tracker.update_status(order_id, OrderState.FAILED)
                    logger.error(f"[EXEC] Order Final Failure {order_id}: {e}", extra={"event": "OrderFailed", "symbol": signal.symbol})
                    return None
    ...
    async def sync_state_on_startup(self):
        ...
        logger.info("[Sync] Synchronizing real exchange state against local StateStore...")
        ...
        for p in live_positions:
            ...
            if sym not in local_positions:
                logger.critical(f"[FAILSAFE] State Mismatch: Exchange has {sym} ({live_amt}), but local DB has none!")
                mismatch_detected = True
            else:
                local_amt = local_positions[sym]['amount']
                # Sub-precision mismatch tolerance
                if abs(live_amt - local_amt) > 0.001:
                    logger.critical(f"[FAILSAFE] Mismatch on {sym}: Exchange ({live_amt}) vs Local ({local_amt})")
                    mismatch_detected = True
        ...
        for sym in local_positions:
            if sym not in live_symbols:
                logger.critical(f"[FAILSAFE] State Mismatch: Local DB tracks {sym}, but Exchange does not have it!")
                mismatch_detected = True
                
        if mismatch_detected:
            logger.critical("[FAILSAFE] TRADING PAUSED. Manual state reconciliation required. Unsafe to proceed.", extra={"event": "StateMismatchHalt"})
            # Hard crash to prevent corrupt trading loops and runaway cascading risk triggers
            import sys
            sys.exit(1)
            
        logger.info("[Sync] State reconciliation complete. DB exactly matches Exchange.")
        return True
    ...
    async def close_all_positions(self):
        ...
        for p in positions:
            ...
            amount = abs(float(p['contracts']))
            logger.warning(f"[EXEC] EMERGENCY CLOSE: {side} {amount} {symbol}", extra={"event": "EmergencyCloseAll", "symbol": symbol})
