    ...
    def execute_signal(self, signal: Signal):
        ...
        if signal.action in [SignalAction.ENTER_LONG, SignalAction.ENTER_SHORT]:
            ...
            if meta and "dca_levels" in meta:
                ...
                for i in range(1, 4):
                    price = (
                        signal.price * (1 - 0.015 * i)
                        if side == "LONG"
                        else signal.price * (1 + 0.015 * i)
                    )
                    dca_levels.append(
                        {"price": price, "amount": signal.amount / 3, "filled": False}
                    )
            ...
            if symbol in self.state["positions"]:
                ...
                pos["amount"] += new_add
                logger.info(
                    f"[PAPER] {symbol} DCA Add at {signal.price}. New Avg: {pos['average_price']:.2f}"
                )
    ...
    def update_positions(self, current_prices: dict):
        ...
        for i, order in enumerate(self.state.get("pending_orders", [])):
            ...
            if fill:
                ...
                logger.info(
                    f"[PAPER] Pending Order Filled: {order['side']} {order['symbol']} at {order['price']}"
                )
        ...
        if orders_to_fill:
            self.state["pending_orders"] = [
                o
                for i, o in enumerate(self.state["pending_orders"])
                if i not in orders_to_fill
            ]
        ...
        for symbol, pos in self.state["positions"].items():
            ...
            if pos["side"] == "LONG":
                if pos.get("stop_loss") and price <= pos["stop_loss"]:
                    ...
                    logger.info(f"[PAPER] {symbol} SL Hit")
                elif pos.get("take_profit") and price >= pos["take_profit"]:
                    closed = True
                    exit_price = pos["take_profit"]
                    logger.info(f"[PAPER] {symbol} TP Hit")
            else:
                if pos.get("stop_loss") and price >= pos["stop_loss"]:
                    closed = True
                    exit_price = pos["stop_loss"]
                    logger.info(f"[PAPER] {symbol} SL Hit (Short)")
                elif pos.get("take_profit") and price <= pos["take_profit"]:
                    closed = True
                    exit_price = pos["take_profit"]
                    logger.info(f"[PAPER] {symbol} TP Hit (Short)")
