...
bot_daily_drawdown_pct = Gauge(
    "bot_daily_drawdown_pct", "Current daily drawdown percentage (negative value)"
)
bot_current_exposure = Gauge(
    "bot_current_exposure", "Total notional exposure currently open in USD"
)
bot_system_health = Gauge(
    "bot_system_health",
    "Global system health (1=OK, 0=KillSwitch Triggered / Safe Mode)",
)
bot_ws_connected = Gauge(
    "bot_ws_connected", "Websocket connection status (1=Connected, 0=Disconnected)"
)
...
def start_metrics_exporter():
    ...
    if getattr(Config, "ENABLE_METRICS", True):
        ...
        try:
            ...
            logger.info(
                "Prometheus Extended Metrics Exporter active",
                extra={"event": "SystemStartup", "symbol": "SYSTEM", "port": port},
            )
        except Exception as e:
            logger.error(
                f"Failed to start Metrics Exporter: {e}",
                extra={"event": "SystemError", "symbol": "SYSTEM"},
            )
