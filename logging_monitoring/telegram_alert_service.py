...
class TelegramAlertService:
    ...
    def __init__(self, bot: Optional[TelegramBot] = None):
        ...
        if not self.bot.enabled:
            logger.info(
                "[ALERTS] Telegram disabled — alerts will only be persisted locally."
            )
        else:
            modes = []
            ...
            if self.status_enabled:
                modes.append("status")
            logger.info(
                "[ALERTS] Telegram active for: %s",
                ", ".join(modes) if modes else "nothing (both flags off)",
            )
    ...
    async def info(
        self, msg: str, dedup_key: Optional[str] = None, force: bool = False
    ):
    ...
    async def critical(self, msg: str, dedup_key: Optional[str] = None):
        ...
        now = time.time()
        if (
            msg == self.last_critical_error
            and (now - self.last_critical_ts) < self.critical_cooldown
        ):
            logger.debug("[ALERTS] Skipping duplicate critical: %s", msg[:50])
            self._persist_alert(
                f"[COOLDOWN SKIP] {msg}", AlertLevel.CRITICAL, dedup_key
            )
            return
    ...
    def _buffer_aggregation(self, message: str, level: AlertLevel):
        ...
        if summary not in self.aggregation_buffer:
            self.aggregation_buffer[summary] = {
                "count": 1,
                "level": level,
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        "[ALERTS] Send failed (attempt %d): %s. Retrying in %ds...",
                        attempt + 1,
                        e,
                        wait_time,
                    )
                    ...
                    logger.error(
                        "[ALERTS] Persistent failure after %d retries: %s",
                        max_retries,
                        e,
                    )
